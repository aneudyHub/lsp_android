...
interface CustomersDao {
    ...
    @Query("SELECT * FROM ${AppDatabase.CUSTOMERS_TABLE_NAME} WHERE documentId LIKE '%' || :keyword || '%' OR name LIKE '%' || :keyword || '%' OR address LIKE '%' || :keyword || '%' OR phoneNumber LIKE '%' || :keyword || '%'")
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertCustomersBatch(customers: List<CustomerEntity>)
